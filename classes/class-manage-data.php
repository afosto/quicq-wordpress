...
if(!class_exists('Wpqai_Manage_Data'))
{
	class Wpqai_Manage_Data
	{
		...
		function quicq_afosto_rewrite_url($images){
			...
			//$quicq_url = get_option( 'wpqai_quicq_afosto_cdn_url' ).'/'.get_option( 'wpqai_quicq_afosto_key' );
			$quicq_url = get_option( 'wpqai_quicq_afosto_cdn_url' );
			
			foreach ( $images as &$imageUrl ) {
				$imageUrl = str_replace( get_option( 'wpqai_quicq_afosto_site_url' ), $quicq_url, $imageUrl );
			}
			return $images[0];
		}
	}
